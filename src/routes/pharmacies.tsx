  ...
  const findNearby = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        fetchNearbyPharmacies(latitude, longitude, 5000)
          .then((pharmacies) => {
            if (pharmacies.length === 0) {
              return fetchNearbyPharmacies(latitude, longitude, 10000);
            }
            return pharmacies;
          })
          .then((pharmacies) => {
            if (pharmacies.length === 0) {
              setMessage(
                "No pharmacies found within 10km. Try visiting nafdac.gov.ng for listings in your state."
              );
            }
      ...
      () => {
        setLocError(
          "Please allow location access in your browser to find nearby pharmacies."
        );
        setLoading(false);
      }
    );
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
        <header className="mb-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Find real pharmacies near you across all 36 Nigerian states. Allow
            location access for the best results.
          </p>
        </header>
        ...
        <form onSubmit={onSubmit} className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full rounded-md border border-input bg-background py-2.5 pl-4 pr-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
          </div>
          <button
            type="button"
