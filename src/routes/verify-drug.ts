...
function rowToResult(row: GBRow, alternatives: string[]) {
  ...
  const cleanName = row.product_name.replaceAll("#", "").replaceAll("*", "").trim();
...
export const APIRoute = createAPIFileRoute("/api/verify-drug")({
  POST: async ({ request }) => {
    ...
    try {
      ...
      if (rows.length > 0) {
        const row = rows[0];
        // Pull alternatives: other registered products sharing the active ingredient
        let alternatives: string[] = [];
        if (row.ingredient?.ingredient_name) {
          const alts = await gbSearch(row.ingredient.ingredient_name, 8);
          const seen = new Set<string>();
          for (const r of alts) {
            const n = r.product_name.replaceAll("#", "").replaceAll("*", "").trim();
            const key = n.toLowerCase();
            if (n && key !== row.product_name.toLowerCase() && !seen.has(key)) {
              seen.add(key);
              alternatives.push(n);
